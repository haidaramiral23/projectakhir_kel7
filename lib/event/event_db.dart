...
static Future<void> UpdateDosen(String dsnNidn, String dsnNama,
  ...
  static Future<List<Eventkampus>> getEventkampus() async {
    List<Eventkampus> ListEventkampus = [];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        var responBody = jsonDecode(response.body);
        if (responBody['success']) {
          var eventkampus = responBody['eventkampus'];

          eventkampus.forEach((Eventkampus) {
            ListEventkampus.add(eventkampus.fromJson(eventkampus));
          });
        }
      }
    } catch (e) {
    ...
    return ListEventkampus;
  }  

  static Future<String> AddEventkampus(String kodeEvent, String namaEvent,
      String deskripsiEvent, String tanggal) async {
    String reason;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        var responBody = jsonDecode(response.body);
        if (responBody['success']) {
          reason = 'Add Event Kampus Berhasil';
        } else {
          reason = responBody['reason'];
        }
      } else {
  ...
  static Future<void> UpdateEventkampus(String kodeEvent, String namaEvent,
      String deskripsiEvent, String tanggal) async {
    try {
      ...
      if (response.statusCode == 200) {
        var responBody = jsonDecode(response.body);
        if (responBody['success']) {
          Info.snackbar('Berhasil Update Eventkampus');
        } else {
          Info.snackbar('Gagal Update Eventkampus');
        }
      }
    } catch (e) {
      print(e);
  ...
  static Future<void> deleteEventkampus(String kodeEventkampus) async {
    try {
      var response = await http
          .post(Uri.parse(Api.deleteEvent), body: {'kodeEvent': kodeEventkampus});

      if (response.statusCode == 200) {
        var responBody = jsonDecode(response.body);
        if (responBody['success']) {
          Info.snackbar('Berhasil Delete Eventkampus');
        } else {
          Info.snackbar('Gagal Delete Eventkampus');
        }
      }
    } catch (e) {
      print(e);
